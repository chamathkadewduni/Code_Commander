...
if(isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tutor_ad WHERE id = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updated_rateType = $_POST["rateType"];
 
    
    // Update the data in the database
    $sql = "UPDATE tutor_ad SET user_id = '$updated_user_id', topic = '$updated_topic', description = '$updated_description', city_id = '$updated_city_id', medium = '$updated_medium', type = '$updated_type', rateType='$updated_rateType', rate='$updated_rate'  WHERE id = $id";
...
<body>
    ...
    <div class="container">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . $id); ?>">
            ...
            <!-- City ID -->
            <div class="mb-3">
                <label for="city_id" class="form-label">Class Conducting Location:</label>
             ...
             <div class="mb-3">
                ...
                <select class="form-select" id="rateType" name="rateType" required>
                    <option <?php if ($rateType == "Hourly") echo "selected"; ?>>Hourly</option>
                    <option <?php if ($rateType == "Daily") echo "selected"; ?>>Daily</option>
                    <option <?php if ($rateType == "Monthly") echo "selected"; ?>>Monthly</option>
                </select>
            </div>
            <!-- Rate -->
              <div class="mb-3">
                <label for="rate" class="form-label">Rate (Rs.):</label>
                <input type="number" class="form-control" id="rate" name="rate" value="<?php echo $rate; ?>" required>
            </div>
            <!-- Medium -->
            <div class="mb-3">
